...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <div className="nm-section-label">{children}</div>
}
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nm-glow {
          ...
          background:
            radial-gradient(ellipse 70% 55% at 15% 85%, rgba(80,100,200,.22) 0%, transparent 70%),
        ...
        .nm-d-icon { width: 44px; height: 44px; border-radius: 11px; background: linear-gradient(135deg,#4f63b8,#8b5bc7); display: flex; align-items: center; justify-content: center; margin-bottom: 10px; }
        ...
        .nm-steps-title { font-size: 11px; font-weight: 500; color: #3a3835; text-transform: uppercase; letter-spacing: .08em; margin-bottom: 1.25rem; }
        .nm-step { display: flex; align-items: flex-start; gap: 12px; margin-bottom: 1.25rem; }
        .nm-step-num { width: 26px; height: 26px; border-radius: 50%; background: linear-gradient(135deg,#4f63b8,#7048be); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 500; color: #fff; flex-shrink: 0; margin-top: 1px; }
        .nm-step-title { font-size: 13px; font-weight: 500; color: #d0cbc4; margin-bottom: 2px; }
        .nm-step-desc { font-size: 11.5px; color: #4a4845; line-height: 1.45; }
        ...
        /* ── Form panel ── */
        .nm-fp { display: flex; align-items: flex-start; justify-content: center; background: #0a0b0f; }
        .nm-fi { width: 100%; padding-top: .5rem; }
        .nm-f-brand { display: flex; align-items: center; gap: 9px; margin-bottom: 1.6rem; text-decoration: none; }
        ...
        .nm-f-h { font-family: 'Instrument Serif', serif; font-style: italic; font-size: 1.75rem; color: #f0ede8; letter-spacing: -.025em; line-height: 1.2; margin-bottom: .3rem; }
        .nm-f-sub { font-size: 12.5px; color: #4a4845; margin-bottom: 1.4rem; }
        ...
        .nm-tab { flex: 1; min-height: 42px; padding: 8px; border-radius: 7px; border: none; font-family: 'DM Sans', sans-serif; font-size: 12.5px; cursor: pointer; color: #4a4845; background: transparent; display: flex; align-items: center; justify-content: center; gap: 5px; transition: all .18s; -webkit-tap-highlight-color: transparent; }
        .nm-tab.active { background: #1c1f2a; color: #d0cbc4; border: 1px solid rgba(255,255,255,.1); box-shadow: 0 1px 4px rgba(0,0,0,.5); }
        .nm-tab-dot { width: 5px; height: 5px; border-radius: 50%; background: #5568cc; opacity: 0; transition: opacity .18s; flex-shrink: 0; }
        .nm-tab.active .nm-tab-dot { opacity: 1; }

        .nm-section-label { font-size: 10px; font-weight: 500; color: #3a3835; text-transform: uppercase; letter-spacing: .09em; margin: 1.1rem 0 .7rem; padding-bottom: .5rem; border-bottom: 1px solid rgba(255,255,255,.06); }

        .nm-field { margin-bottom: .8rem; }
        ...
        .nm-inp {
          ...
          padding: 11px 13px; min-height: 46px; font-family: 'DM Sans', sans-serif; font-size: 14px; color: #e0dbd3;
          outline: none; transition: border-color .15s, box-shadow .15s;
        }
        .nm-inp::placeholder { color: #2a2826; }
        .nm-inp:focus { border-color: rgba(99,120,200,.55); box-shadow: 0 0 0 3px rgba(99,120,200,.11); }
        ...
        .nm-sel {
          width: 100%; background: #111318; border: 1px solid rgba(255,255,255,.09); border-radius: 9px;
          padding: 11px 36px 11px 13px; min-height: 46px; font-family: 'DM Sans', sans-serif; font-size: 14px; color: #e0dbd3;
        ...
        .nm-sel:focus { border-color: rgba(99,120,200,.55); box-shadow: 0 0 0 3px rgba(99,120,200,.11); }
        .nm-sel option { background: #1a1d26; color: #e0dbd3; }
        .nm-row2 { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: .8rem; }

        .nm-verify-note { background: #13161e; border: 1px solid rgba(99,120,200,.2); border-radius: 9px; padding: 12px 14px; margin: .5rem 0 .9rem; display: flex; gap: 10px; align-items: flex-start; }
        .nm-verify-dot { width: 6px; height: 6px; border-radius: 50%; background: #5568cc; flex-shrink: 0; margin-top: 4px; }
        .nm-verify-text { font-size: 11.5px; color: #5568cc; line-height: 1.5; }

        .nm-checks { margin: .5rem 0 1rem; }
        ...
        .nm-check-lbl { font-size: 12px; color: #4a4845; line-height: 1.4; }
        ...
        .nm-btn {
          ...
          letter-spacing: .01em; transition: opacity .15s, transform .12s;
          position: relative; overflow: hidden; -webkit-tap-highlight-color: transparent;
        }
        .nm-btn::before { content: ''; position: absolute; inset: 0; background: linear-gradient(to bottom, rgba(255,255,255,.09), transparent); pointer-events: none; }
        .nm-btn:hover:not(:disabled) { opacity: .9; }
        ...
        .nm-trust { display: flex; align-items: center; justify-content: center; flex-wrap: wrap; gap: 5px; font-size: 10px; color: #2a2825; margin-top: .9rem; }
      ...
      <div className="nm-root">
        ...
        <div className="nm-mob-hero">
          ...
          <div className="nm-m-h">Create your account.</div>
          <div className="nm-m-sub">Get started with AI-powered legal assistance</div>
        ...
        <div className="nm-fp">
          <div className="nm-fi">
            ...
            <h1 className="nm-f-h nm-fade nm-d2">Create your account.</h1>
            <p className="nm-f-sub nm-fade nm-d2">Choose your role and fill in your details below</p>
            ...
            <form onSubmit={handleSubmit}>
              {userType === "user" ? (
                <>
                  <Field label="Full name"><TextInput value={user.fullName} onChange={v => setU("fullName", v)} placeholder="Priya Sharma" autoComplete="name" required /></Field>
                  <Field label="Email address"><TextInput type="email" value={user.email} onChange={v => setU("email", v)} placeholder="you@example.com" inputMode="email" autoComplete="email" required /></Field>
                  <Field label="Phone number"><TextInput type="tel" value={user.phone} onChange={v => { if (/^\d{0,10}$/.test(v)) setU("phone", v) }} placeholder="9876543210" inputMode="numeric" required /></Field>
                  <Field label="Password"><PasswordInput id="upw" value={user.password} onChange={v => setU("password", v)} placeholder="Create a strong password" /></Field>
                  <Field label="Confirm password"><PasswordInput id="ucpw" value={user.confirmPassword} onChange={v => setU("confirmPassword", v)} placeholder="Repeat your password" /></Field>
                ...
                <>
                  ...
                  <Field label="Full name"><TextInput value={lawyer.fullName} onChange={v => setL("fullName", v)} placeholder="Advocate Rajesh Kumar" autoComplete="name" required /></Field>
                  <div className="nm-row2">
                    <Field label="Email address"><TextInput type="email" value={lawyer.email} onChange={v => setL("email", v)} placeholder="you@example.com" required /></Field>
                    <Field label="Phone number"><TextInput type="tel" value={lawyer.phone} onChange={v => { if (/^\d{0,10}$/.test(v)) setL("phone", v) }} placeholder="9876543210" required /></Field>
                  </div>
                  <Field label="Bar council registration number"><TextInput value={lawyer.barCouncilNumber} onChange={v => setL("barCouncilNumber", v)} placeholder="e.g. MH/1234/2010" required /></Field>

                  <SectionLabel>Professional details</SectionLabel>
                  <div className="nm-row2">
                    <Field label="Primary specialization"><SelectInput value={lawyer.specialization} onChange={v => setL("specialization", v)} options={SPECIALIZATIONS} placeholder="Select specialization" /></Field>
                    <Field label="Years of experience"><SelectInput value={lawyer.experience} onChange={v => setL("experience", v)} options={Array.from({ length: 41 }, (_, i) => `${i} ${i === 1 ? "year" : "years"}`)} placeholder="Select experience" /></Field>
                  </div>
                  <div className="nm-row2">
                    <Field label="State"><SelectInput value={lawyer.state} onChange={v => setL("state", v)} options={INDIAN_STATES} placeholder="Select state" /></Field>
                    <Field label="City"><TextInput value={lawyer.city} onChange={v => setL("city", v)} placeholder="Mumbai" required /></Field>
                  </div>

                  <SectionLabel>Security</SectionLabel>
                  <Field label="Password"><PasswordInput id="lpw" value={lawyer.password} onChange={v => setL("password", v)} placeholder="Create a strong password" /></Field>
                  <Field label="Confirm password"><PasswordInput id="lcpw" value={lawyer.confirmPassword} onChange={v => setL("confirmPassword", v)} placeholder="Repeat your password" /></Field>

                  <div className="nm-verify-note">
                    <div className="nm-verify-dot" />
